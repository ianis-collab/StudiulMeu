/* ============================================
   StudiuMeu – DISCURS BIBLIC PUBLIC (30 MIN)
   Gestionează secțiunile, timerul de 30 min și lista
   cuvântărilor salvate.
...
function getDiscursSections(source) {
  ...
  return sections.map(s => ({
    title:    String(s.title || '').trim() || 'Subtitlu fără titlu',
...
function getDiscursTotalSeconds() {
  return getCurrentDiscursSections().reduce((sum, s) => sum + s.duration * 60, 0);
}
...
function getDiscursSectionInfo(elapsedSeconds = discursTimerElapsedSeconds) {
  ...
  for (let i = 0; i < sections.length; i++) {
    const durSec   = sections[i].duration * 60;
    const sectionEnd = cursor + durSec;
    if (elapsedSeconds < sectionEnd || i === sections.length - 1) {
      return { index: i, section: sections[i], sectionStart: cursor, sectionEnd, remaining: Math.max(0, sectionEnd - elapsedSeconds) };
    }
    cursor = sectionEnd;
  }
...
function removeDiscursSection(index) {
  const sections = getCurrentDiscursSections();
  if (sections.length <= 1) { showToast('Păstrează cel puțin un subtitlu.', 'error'); return; }
...
function saveDiscursNote() {
  ...
  if (talk) {
    talk.data = data; talk.titlu = titlu; talk.verset = verset; talk.note = note; talk.sections = sections;
  } else {
    talk = { id: Date.now().toString(), data, titlu, verset, note, sections };
    state.discursTalks.push(talk);
  }

  state.discursDraft = {
    id: talk.id, data: talk.data, titlu: talk.titlu,
    verset: talk.verset, note: talk.note, sections: getDiscursSections(talk.sections),
  };
  rememberDiscursDraftForDay(state.discursDraft);
  markStudyDay();
  saveState();
...
function renderDiscursTalksList() {
  ...
  container.innerHTML = sorted.map(talk => {
    const minutes = getDiscursSections(talk.sections).reduce((sum, s) => sum + s.duration, 0);
    return `
...
function discursTimerStart() {
  ...
  discursTimerInterval = setInterval(function () {
    const totalSeconds = getDiscursTotalSeconds();
    if (discursTimerElapsedSeconds >= totalSeconds) { discursTimerFinish(); return; }
...
function discursShowTimerAlert(message, shouldVibrate) {
  ...
  if (alertEl) { alertEl.textContent = message; alertEl.classList.add('show'); }
  if (timerBar) {
