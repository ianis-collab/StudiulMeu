/* ============================================
   StudiuMeu – TURNUL DE VEGHE (STUDIU)
   Gestionează paragrafelee, sinteza, salvarea studiului
   și asistentul AI pentru sugestii de comentarii.
...
function autoSynthesize() {
  ...
  wtParagraphs.forEach(p => { if (p.main) synthesis += `• ${p.main}\n`; });

  synthesis += `\nRăspunsuri pregătite:\n`;
  wtParagraphs.forEach(p => { if (p.answer) synthesis += `§${p.number}: ${p.answer}\n\n`; });
...
// ============================================
// AI ASISTENT – SUGESTII COMENTARII
// ============================================
...
function generateTemplates(mainText, verseText) {
  ...
  if (cleanedMain.endsWith('.')) cleanedMain = cleanedMain.slice(0, -1);
